...
metadata {
    ...
    preferences {
        ...
        input name: "p5", type: "number", title: "5. Dimmer min level", range: "0..99", defaultValue: 0
        input name: "p6", type: "number", title: "6. Dimmer max level", range: "1..99", defaultValue: 99
...
def configure() {
    ...
    parent.setParameter(parameterNumber = 5, size = 1, value = p5.toInteger())
    parent.setParameter(parameterNumber = 6, size = 1, value = p6.toInteger())
    if (!prestaging) {
...
void setLevel(level, duration = 0) {
    ...
    if (prestaging) {
        ...
        parent.setParameter(parameterNumber = 10 + prestage_button * 8, size = 4, value = get_dimmer_on_value(level as Integer))
    }
